...
def dit_ideal_generators_prime(K, p):
    r"""
    ...
    EXAMPLES:
        ...
        sage: dit_ideal_generators_prime(K, 2)
        [1, 16, 256, 4096]
        sage: dit_ideal_generators_prime(K, 5)
        [1, -2*a^2 + 19*a + 7, 1, -3*a^2 + a - 1]
        sage: dit_ideal_generators_prime(QQ, 3)
        [1, 729]
    """
...
def dit_ideal_generators_square(K, n):
    r"""
    Returns a list of lambdas, number field elements such that for each fp dividing p,
    where n = p^2, and for each 0 \leq k \leq nup(p), fp^sk = lambda OK.

    INPUT:

    - ``K`` -- number field
    - ``n`` -- a prime squared from the set [9, 25]
    ...
    EXAMPLES:
        ...
        sage: P.<x> = QQ[]
        sage: K.<a> = NumberField(x^3 - 2)
        sage: dit_ideal_generators_square(K, 9)
        [1, 3, 9, 27]
        sage: dit_ideal_generators_square(K, 25)
        [1, a^2 - 2*a - 1, 1, -a^2 - 1]
        sage: dit_ideal_generators_square(QQ, 9)
        [1, 27]
    """
    if n not in [9, 25]:
        raise NotImplementedError('Only implemented for n in [9, 25].')
    p = ZZ(sqrt(n))
    if n == 9: sub = 3
    elif n == 25: sub = 1
    if K == QQ:
...
def dit_ideal_generators(K, n):
    r"""
    Returns a list of lambdas, number field elements such that for each
    fp dividing n and for each 0 < k \leq e, fp^(mk) = lambda OK.
    """
    if ZZ(n).is_prime():
        return dit_ideal_generators_prime(K, n)
    elif ZZ(n).is_square():
        return dit_ideal_generators_square(K, n)
    else:
        raise ValueError
...
def is_singular_value(p, t0):
    ...
    EXAMPLES:
        ...
        True
    """
    if p not in _genus0_primes and p not in [9, 25]:
        raise NotImplementedError('Only implemented for p in [2, 3, 5, 7, 13].')
    ...
    elif p == 9 and (t0^2 + 3*t0 + 9)*(t0 - 3) == 0: return True
    elif p == 25 and (t0^4 + t0^3 + 6*t0^2 + 6*t0 + 11)*(t0^2 + 4)^3*(t0 - 1) == 0: return True
    else: return False
...
def discriminant_ideal_twins_nonsingular_j_invariants_squares(K):
    r"""
    Returns a list of pairs of discriminant ideal twins, where the j-invariants
    are not both 0 or 1728.
    ...
    if K == QQ:
        ...
        units = K.roots_of_unity()
    T0 = [dit_ideal_generators_square(K, n) for n in [9, 25]]
    for p, T0p in zip([3, 5], T0):
        if p == 3: m = 3
        elif p == 5: m = 1
        pm = p^m
        for lam, u in product(T0p, units):
            if lam*u in [pm, -pm]:
                # These are twists of 1, -1, so we can skip them
                continue
            t0 = lam*u + m
            #print(p, t0, lam, u)
            if is_singular_value(p^2, t0): continue
            E1, E2, E3 = isog_curves(p^2, t0, 1)
            if E1.minimal_discriminant_ideal() == E3.minimal_discriminant_ideal():
                if E1.is_isomorphic(E3) or E1.isogeny_degree(E3) == 1:
                    # I'm being a bit lazy here.  I could use the results from section 3, Theorem 3.9
                    # to preimptively remove these, or I could just check on the go.
                    continue
                else:
                    DiscIdealTwins.append([E1, E3])
            else:
                Warning = "Should have discriminant ideal twins, but don't."
                assert E1.minimal_discriminant_ideal() == E3.minimal_discriminant_ideal(), (Warning)
    return DiscIdealTwins
